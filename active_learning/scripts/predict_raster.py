...
Backends (`--backend`):
  ...
  timber  placeholder; raises (timber-compiled inference is a net loss on this batched
          workload — see active_learning/reports/timber_report.md). Seam kept for a
          future single-sample / edge deployment.

Usage:
  ~/myprojects/recover/.venv/bin/python active_learning/scripts/predict_raster.py
  ~/myprojects/recover/.venv/bin/python active_learning/scripts/predict_raster.py \
      --tile /path/to/embeddings_XXXX.tif --out-dir active_learning/reports
"""
...
DEFAULT_OUT = mc.AL_ROOT / "reports"
...
def predict_tile(tile: Path, seed: mc.SeedModel, backend: str,
    ...
    if backend == "timber":
        raise NotImplementedError(
            ...
            "(see active_learning/reports/timber_report.md). Use --backend native.")
