"""Scheme B round-4c: TabICL as Stage-1 pseudo-labeller.

Compares three Stage-1 strategies for generating pseudo-labels on unstable data,
all followed by the same Stage-2: TabICL(n_est=16, kv_cache=True, 25k support).

Conditions:
  cb_stage1_tabicl_stage2      CatBoost-500 CPU as Stage 1 (current best, baseline)
  tabicl_stage1_n8_tabicl_s2   TabICL(n_est=8,  kv_cache=True) as Stage 1
  tabicl_stage1_n16_tabicl_s2  TabICL(n_est=16, kv_cache=True) as Stage 1

GPU note: Stage-1 TabICL and Stage-2 TabICL are never in GPU memory simultaneously.
The Stage-1 model is deleted and cache cleared before Stage-2 begins.

Output:
  common_ground/reports/research/schemeB_round4_tabicl_stage1_results.json
"""
...
def make_tabicl(n_estimators, kv_cache=True):
    from tabicl import TabICLClassifier
    return TabICLClassifier(
        n_estimators=n_estimators, kv_cache=kv_cache,
...
def run():
    ...
    print(f"device={device}  n_folds={N_FOLDS}  scheme=B  round=4c (TabICL Stage-1)")
    ...
    conditions = [
        ("cb_stage1_tabicl_stage2",
         "CatBoost-500 CPU Stage-1 + TabICL n16 kvcache Stage-2 (25k support)"),
        ("tabicl_stage1_n8_nokv_tabicl_s2",
         "TabICL n8 kv_cache=False Stage-1 + TabICL n16 kvcache Stage-2 (25k support)"),
        ("tabicl_stage1_n16_nokv_tabicl_s2",
         "TabICL n16 kv_cache=False Stage-1 + TabICL n16 kvcache Stage-2 (25k support)"),
    ]
    # kv_cache=False for Stage-1: Stage-1 only needs pseudo-label argmax, not fast inference.
    # kv_cache=True for Stage-2 only. This avoids the OOM from building two large KV caches.
    ...
    for k in range(N_FOLDS):
        ...
        for cond_name, cond_desc in conditions:
            ...
            # ── Stage 1 ──────────────────────────────────────────────
            if cond_name == "cb_stage1_tabicl_stage2":
                ...
                n_est_s1 = 8 if "_n8_" in cond_name else 16
                stage1 = make_tabicl(n_est_s1, kv_cache=False)
                t_s1 = time.perf_counter()
                stage1.fit(X_s_tr, y_s_tr)
                t_s1_fit = time.perf_counter() - t_s1
                proba_u = stage1.predict_proba(X_u_tr).astype(np.float32)
                pseudo_u = proba_u.argmax(axis=1)
                # mean Stage-1 pseudo confidence per class
                top1_conf = proba_u.max(axis=1)
                print(f"    Stage-1 TabICL n_est={n_est_s1}  fit={t_s1_fit:.1f}s  "
                      f"mean_conf={top1_conf.mean():.3f}  "
                      f"low_conf(<0.6)={100*(top1_conf<0.6).mean():.1f}%")
                del stage1
                clear_cuda()

            # log pseudo-label class distribution
            counts = np.bincount(pseudo_u, minlength=n_classes)
            weak_enc = [le.transform([c])[0] for c in [2, 5, 7] if c in le.classes_]
            weak_info = "  ".join(
                f"cls{decoded[c]}={counts[c]}" for c in weak_enc if c < len(counts))
            print(f"    pseudo-label counts: total={len(pseudo_u)}  {weak_info}")

            # ── Stage 2 ───────────────────────────────────────────────
            X_aug = np.concatenate([X_s_tr, X_u_tr])
            y_aug = np.concatenate([y_s_tr, pseudo_u])
            X_sup, y_sup = random_subsample(X_aug, y_aug, SUP_SIZE, rng)

            clear_cuda()
            try:
                stage2 = make_tabicl(16)
                t_fit = time.perf_counter()
                stage2.fit(X_sup, y_sup)
                t_fit = time.perf_counter() - t_fit
                t_pred = time.perf_counter()
                # batch predict_proba to avoid OOM on large test sets
                PRED_BATCH = 10_000
                probs_parts = []
                for i in range(0, len(X_s_te), PRED_BATCH):
                    probs_parts.append(
                        stage2.predict_proba(X_s_te[i:i+PRED_BATCH]).astype(np.float64))
                    clear_cuda()
                ...
                del stage2
                print(f"  [{cond_name:<36}] F1={f1m:.4f}  bal={bal:.4f}  "
                      f"stage2_fit={t_fit:.1f}s  pred={t_pred:.1f}s  n_sup={len(y_sup):,}")
                _record(results, descs, cond_name, cond_desc,
                        k, f1m, bal, pc, t_fit, t_pred, len(y_sup))
            except Exception as e:
                print(f"  [{cond_name:<36}] FAILED: {type(e).__name__}: {str(e)[:120]}")
    ...
    out_json = OUT_DIR / "schemeB_round4_tabicl_stage1_results.json"
    ...
    # ── leaderboard ──────────────────────────────────────────────────
    weak_classes = [2, 5, 7]
    weak_enc = {c: i for i, c in enumerate(decoded) if c in weak_classes}

    print(f"\n{'Condition':<38}  {'F1':>8}  {'std':>6}  {'Δ ref':>8}")
    print("-" * 66)
    for name, s in sorted(summary.items(), key=lambda x: -(x[1]["f1_mean"] if not np.isnan(x[1]["f1_mean"]) else -1)):
        if np.isnan(s["f1_mean"]):
            print(f"{name:<38}  {'NaN':>8}  {'NaN':>6}  [FAIL]")
        else:
            d = s["f1_mean"] - REF_BEST
            flag = " ★" if d > 0.003 else ""
            print(f"{name:<38}  {s['f1_mean']:>8.4f}  {s['f1_std']:>6.4f}  {d:>+8.4f}{flag}")

    print(f"\nPer-class F1 for weak classes (classes 2, 5, 7):")
    print(f"{'Condition':<38}  " + "  ".join(f"cls{c:>2}" for c in weak_classes))
    print("-" * 66)
